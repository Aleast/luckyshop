...
class Role extends Base {
    ...
    public function add()
    {
        $data['pids'] = $this->role_model->get_list();
        // var_dump($mids);exit;
    ...
    public function addone()
    {
        ...
        if($this->role_model->add()>0){

            $data['status']="true";


        }elseif($this->role_model->add()==-1){

            $data['status']="部门名称已存在";


        }elseif($this->role_model->add()==-2){
    ...
    public function edit()
    {
        ...
        // $data['rl']  = $this->role_model->get_info($data['info']->phone);
        // var_dump($data);exit;
    ...
    public function update()
    {
        ...
        if($this->role_model->update()>0){
            $data['status']="true";
        }elseif($this->role_model->update()==-1){
    ...
    public function delall()
    {
        ...
        if($this->role_model->del()>0){
            $data['status']="true";
        }elseif($this->role_model->del()==-1){
            $data['status']="部门下仍有子部门，不能删除!";

        }elseif($this->role_model->del()==-2){
